...
class UnifiedMemoryManager:
    ...
    async def initialize(self):
        ...
        if not self.pool:
            print("[Pamięć] Nawiązuję asynchroniczne połączenie z PostgreSQL...")
            self.pool = await asyncpg.create_pool(settings.database_url)
            
            async with self.pool.acquire() as conn:
                print("[Pamięć] Sprawdzam i migruję strukturę bazy danych...")
                await conn.execute("CREATE EXTENSION IF NOT EXISTS vector;")
                
                # Tabela główna agent_memory (Zarządza indeksowaniem ścieżek L3 oraz L4)
                await conn.execute("""
                ...
                try:
                    ...
                    print(f"[Pamięć Migracja] Info UNIQUE: {sql_err}")
                
                # Tabela L1/L2 historycznego zapisu sesji
                await conn.execute("""
            ...
            # Upewniamy się, że foldery na dysku fizycznie istnieją
            ...
            print("[Pamięć] Pełna struktura Enterprise L0-L4 zainicjalizowana pomyślnie.")
    ...
    async def save_to_local_notes_and_vector(self, rel_path: str, content: str, metadata: Dict[str, Any]):
        ...
        # Zapis fizycznego pliku w brudnopisie bota (L3)
        ...
        # Trwały zapis w bazie danych (L2) z oznaczeniem poziomu pamięci
        ...
        print(f"📌 [Pamięć L3 + L2] Zsynchronizowano wewnętrzny brudnopis operacyjny: {rel_path}")
